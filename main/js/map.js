...
function loadAdminBoundariesFromGeoJSON(geojsonData) {
    try {
        ...
        // adminBoundaryLayer.addTo(map); // Don't add to map by default, but keep it in layer control
    ...
    try {
        ...
        stations.forEach(station => {
            if (station.latitude && station.longitude) {
                let order = station.order_of_accuracy || station.order_of_accuracy_wgs84 || station.order || station.elevation_order || station.horizontal_order || '';
...
function updateMapMarkers(points, mapInstance = map) {
    try {
        if (!mapInstance) {
        ...
        // Ensure points is an array before proceeding
        if (!Array.isArray(points)) {
            console.error('updateMapMarkers received non-array points:', points);
            // Clear existing markers if points is not an array
            if (markersClusterGroup) {
                markersClusterGroup.clearLayers();
            }
            return;
        ...
        points.forEach(point => {
            if (point.latitude && point.longitude) {
                ...
                const marker = L.marker([point.latitude, point.longitude], {
                    ...
                    Lat: ${Number(point.latitude).toFixed(3)}<br>
                    Long: ${Number(point.longitude).toFixed(3)}<br>
                    Order: ${point.order_of_accuracy_wgs84 || 'N/A'}<br>
                    Project: ${point.project || 'N/A'}<br>
                    Epoch: ${point.epoch === '0.0' || point.epoch === 'N/A' ? '' : point.epoch}<br>
                    <div class="mt-2" style="display: flex; align-items: center; gap: 10px;">
        ...
        if (hasValidCoordinates && mapInstance) {
        ...
        showError('Failed to update map markers: ' + error.message);
    }
}
