...
// Function to calculate the angle in a triangle given the lengths of the sides
...
// Function to find the best triangle
function findBestTriangle(clickedLat, clickedLon, greenLat, greenLon) {
    var markers = [];
    map.eachLayer(function(layer) {
        if (layer instanceof L.Marker && layer.options.icon.options.iconUrl === 'https://maps.google.com/mapfiles/ms/icons/orange-dot.png') {
            var markerLat = layer.getLatLng().lat;
            var markerLon = layer.getLatLng().lng;
            markers.push({ lat: markerLat, lon: markerLon, layer: layer });
        }
    });
    ...
    markers.forEach(function(marker) {
        var orangeLat = marker.lat;
        var orangeLon = marker.lon;

        var a = calculateDistance(greenLat, greenLon, clickedLat, clickedLon);
        var b = calculateDistance(greenLat, greenLon, orangeLat, orangeLon);
        var c = calculateDistance(clickedLat, clickedLon, orangeLat, orangeLon);

        var angles = calculateAngles(a, b, c);

        // Calculate the total difference from 60 degrees for each angle
        var angleDiff = Math.abs(60 - angles[0]) + Math.abs(60 - angles[1]) + Math.abs(60 - angles[2]);

        if (angleDiff < minAngleDiff) {
            minAngleDiff = angleDiff;
            bestTriangle = [greenLat, greenLon, clickedLat, clickedLon, orangeLat, orangeLon];
        }
    });
...
// Add double click event listener to orange markers
map.on('dblclick', function(e) {
    var clickedLat = e.latlng.lat;
    var clickedLon = e.latlng.lng;

    // Green marker coordinates
    var greenLat = greenMarker.getLatLng().lat;
    var greenLon = greenMarker.getLatLng().lng;

    // Find the best triangle
    var bestTriangle = findBestTriangle(clickedLat, clickedLon, greenLat, greenLon);

    if (bestTriangle) {
        var lineCoordinates = [
            [bestTriangle[0], bestTriangle[1]], // Green marker
            [bestTriangle[2], bestTriangle[3]], // Clicked marker
            [bestTriangle[4], bestTriangle[5]]  // Closest marker to form the triangle
        ];

        // Draw the triangle
        L.polygon(lineCoordinates, { color: 'blue' }).addTo(map);
    } else {
        alert("No valid triangle found.");
    }
});
