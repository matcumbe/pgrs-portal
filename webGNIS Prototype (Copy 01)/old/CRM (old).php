...
<div class="container">
    ...
    <table id="requestsTable" class="scrollable-table">
        ...
        $sql = "SELECT ID, client, email, stat_names FROM $table";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            // Output data of each row
            while($row = $result->fetch_assoc()) {
                echo "<tr ondblclick='addToSelected(\"" . $row["ID"] . "\")'>
                        <td>" . $row["ID"] . "</td>
                        <td>" . $row["client"] . "</td>
                        <td>" . $row["email"] . "</td>
...
function addToSelected(requestId) {
    if (!selectedRequests.includes(requestId)) {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            if (cols.length > 0 && cols[0].innerText === requestId) {
                var newRow = rows[i].cloneNode(true);
                newRow.ondblclick = function() { removeFromSelected(requestId); };
...
function removeFromSelected(requestId) {
    ...
    for (var i = 1; i < rows.length; i++) {
        ...
        if (rowData[0].textContent === id) {
            return {
                ...
                client: rowData[1].textContent,
                email: rowData[2].textContent,
                stat_names: rowData[3].textContent
            };
        }
    }
    return null;
        ...
        if (request) {
            ...
            statNames.forEach(function(statName) {
                ...
                stations.forEach(function(station) {
                    ...
                    row.insertCell().textContent = station.latitude;
                    row.insertCell().textContent = station.longitude;
                });
            });
            ...
            generateButton.onclick = (function(clientName, requestId, button) {
                return function() {
                    generatePDF(clientName, requestId, button);
                };
            })(clientName, requestId, generateButton);
...
function generatePDF(clientName, requestId, button) {
    // Get data for each station
    var stationsData = [];
    var selectedRequest = getRequestById(requestId);
