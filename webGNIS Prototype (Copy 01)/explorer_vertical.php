...
<title>GNIS | Explorer (Vertical)</title>
...
<div id="map-container" class="container">
    ...
    <div id="map"></div>
        <div class="filter-container">
            <label><input type="checkbox" id="order1" checked> 1 CM</label>
            <label><input type="checkbox" id="order2" checked> 2 CM</label>
        </div>
...
try {
    ...
    $stmt = $pdo->query("SELECT ID, stat_name, status, island, region, province, municipal, barangay, N84dd, E84dd, order_acc, N92d, N92m, N92s, E92d, E92m, E92s, H92, N92ptm, E92ptm, Z92, N84d, N84m, N84s, E84d, E84m, E84s, H84, descripts, E92utm, N92utm, Z92utm, E84utm, N84utm, Z84utm, accuracy_class FROM gcp_table");
    ...
    while ($row = $stmt_available->fetch(PDO::FETCH_ASSOC)) {
        if (!empty($row['accuracy_class'])) {
            ...
            $available_points_table .= '<td>' . htmlspecialchars($row['accuracy_class']) . '</td>';
            $available_points_table .= '</tr>';
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    const updateMarkers = () => {
        ...
        completeInformation.forEach(point => {
            ...
            const order = parseInt(point.accuracy_class);
            let icon;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    submitBtn.addEventListener('click', function() {
        ...
        xhr.open('POST', 'explorer_v_submit_request.php', true);
...
selectedPoints.forEach((point, index) => {
    ...
    if (pointInfo) {
        ...
        // Add rows to the table according to the new layout
        popupWindow.document.write('<tr><th>Station Name:</th><td colspan="3">' + pointInfo.stat_name + '</td></tr>');
        popupWindow.document.write('<tr><th>Island:</th><td>' + pointInfo.island + '</td><th>Province:</th><td>' + pointInfo.province + '</td></tr>');
        popupWindow.document.write('<tr><th>Municipality:</th><td>' + pointInfo.municipal + '</td><th>Barangay:</th><td>' + pointInfo.barangay + '</td></tr>');
        popupWindow.document.write('<tr><th>Ellipsoidal Height (m):</th><td>' + pointInfo.H84 + '</td><th>Accuracy class (CM):</th><td>' + pointInfo.accuracy_class + '</td></tr>');
        popupWindow.document.write('<tr><th>Latitude:</th><td>' + pointInfo.N84d + '° ' + pointInfo.N84m + '\' ' + pointInfo.N84s + '"</td><th>Longitude:</th><td>' + pointInfo.E84d + '° ' + pointInfo.E84m + '\' ' + pointInfo.E84s + '"</td></tr>');
        popupWindow.document.write('<tr><td colspan="4">' + pointInfo.descripts + '</td></tr>');
    ...
    window.addEventListener("DOMContentLoaded", (event) => {
        const dlSHP = document.getElementById('download-shp-btn');
        if (dlSHP) {
        dlSHP.addEventListener('click', shpDL, false);
        }
    });

    function shpDL() {
        let stationNames = [];
        const rows = document.querySelectorAll('#selected-table tbody tr');
        rows.forEach(row => {
            stationNames.push(row.cells[0].textContent);
        });

        fetch('explorer_generateSHP.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ stationNames: stationNames })
        })
        ...
        .then(blob => {
            ...
            const formattedDate = currentDate.toISOString().replace(/[-:.]/g, "").slice(0, 15); // Format: YYYYMMDDTHHMMSS
            a.download = `GNISOnline_${formattedDate}.zip`;
...
<div class="button-container">
    ...
    <button id="download-shp-btn">Download Shapefile</button>
