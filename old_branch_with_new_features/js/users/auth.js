...
function updateNavigation() {
    ...
    if (currentUser) {
        ...
        adminLinks.forEach(link => {
            if (currentUser.user_type === 'admin' || currentUser.user_type === 'moderator') {
                // Check if this is the users management link and user is moderator
                if (currentUser.user_type === 'moderator' && link.href && link.href.includes('users_management')) {
                    link.classList.add('d-none'); // Hide users management for moderators
                } else {
                    link.classList.remove('d-none');
                }
                
                // Hide Home, Tracker, and About links for admin/moderator users
...
// Protect admin-only pages (like users management)
function protectAdminOnlyPage() {
...
function hasRole(requiredRole) {
    ...
    // Moderator can access everything except users management
    if (currentUser.user_type === 'moderator') {
        return requiredRole !== 'users_management';
    }
...
function hasAdminAccess() {
    return currentUser && (currentUser.user_type === 'admin' || currentUser.user_type === 'moderator');
}

// Check if user has admin-only access (admin only)
function hasAdminOnlyAccess() {
