...
CREATE OR REPLACE VIEW vw_missing_coordinates_2 AS
...
CREATE OR REPLACE VIEW vw_inconsistent_status_2 AS
...
-- View for duplicate station names (simplified version)
CREATE OR REPLACE VIEW vw_duplicate_names_2 AS
SELECT station_name, COUNT(*) as count
FROM stations
GROUP BY station_name
HAVING COUNT(*) > 1;

-- View for stations with multiple observations (detailed version)
CREATE OR REPLACE VIEW vw_multiple_observations_2 AS
...
CREATE OR REPLACE VIEW vw_coordinate_discrepancies_2 AS
SELECT s.station_id, s.station_name,
...
CREATE OR REPLACE VIEW vw_missing_admin_data_2 AS
SELECT s.station_id, s.station_name,
...
CREATE OR REPLACE VIEW vw_temporal_inconsistencies_2 AS
SELECT s.station_id, s.station_name,
...
CREATE OR REPLACE VIEW vw_accuracy_issues_2 AS
SELECT s.station_id, s.station_name,
       sc.accuracy_class,
       sc.horizontal_error,
       sc.vertical_error,
...
CREATE OR REPLACE VIEW vw_description_issues_2 AS
SELECT s.station_id, s.station_name,
       CASE 
